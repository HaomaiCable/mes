var YDataGrid = function(config){
		...
		var Handler = {
			...
			search: function(callback){
			    //$("#xhcombobox").combobox('loadData', #{unixh));
				Events.refresh();
				//回调函数
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					YiYa.getById(Action.getId,data,function(result){
					    ...
					    $("#dg-xsddappend").datagrid('loadData', result.json); //新增销售订单
					    $("#dg-xsddwghb").datagrid('loadData', result.json); //质量部完工汇报
					    $("#dg-xsddrkhb").datagrid('loadData', result.json); //统计入库汇报
						$("#dg-jtjhappend").datagrid('loadData', result.json); //机台计划下达
						$("#dg-jtjhwghb").datagrid('loadData', result.json); //机台计划完工汇报
						$("#dg-bcpjhappend").datagrid('loadData', result.json); //半成品计划下达
						$("#dg-zbggappend").datagrid('loadData', result.json); //采购招标公告发布
                        $("#dg-zbconfirm").datagrid('loadData', result.json); //中标供应商确认
			...
			remove: function(callback){
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('提示','确认要删除选定记录吗?',function(r){  
					    if (r){
					    	var idKey = dataGrid.idField || 'id'; //主键名称
			...
			close : function (callback){
				$.messager.confirm('提示','确认要关闭窗口吗?',function(r){  
				    if (r){  
				     	Win.edit.dialog('close');
				     	//回调函数
						if(jQuery.isFunction(callback)){
		...
		var initGrid = function(){
			var dataconfig = {
				...
				nowrap: dataGrid.nowrap || false,
				autoRowHeight: false,
				striped: true,
				collapsible:true,
				remoteSort: false,
				pagination:dataGrid.pagination || true,
				pageSize:dataGrid.pageSize ||10,
			    pageList:dataGrid.pageList || [10, 20, 30, 40, 50],
				rownumbers:true,
                ...
                frozenColumns: dataGrid.frozenColumns,
